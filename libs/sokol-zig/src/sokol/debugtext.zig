...
pub fn print(comptime fmt: []const u8, args: anytype) void {
    ...
    //TODO: that's kinda bad practice but uhhh...
    var buf = std.heap.page_allocator.alloc(u8, size + 1) catch unreachable;
    defer std.heap.page_allocator.free(buf);
    const str = std.fmt.bufPrintZ(buf, fmt, args) catch unreachable;
    puts(str);
    crlf();
}
